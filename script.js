document.addEventListener('DOMContentLoaded', () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                resultImage.src = url;
                    ...
                    if (lastResultUrl) {
                        try { URL.revokeObjectURL(lastResultUrl); } catch (e) {}
                    }
                    ...
                    // trigger immediate download of the generated image
                    downloadUrl(url, 'background-removed.png');
            } else {
    ...
    const downloadUrl = (url, filename = 'background-removed.png') => {
    ...
    freeDownloadBtn.addEventListener('click', async (e) => {
        // If a result already exists, download it instead of re-processing
        if (lastResultUrl) {
            downloadUrl(lastResultUrl, 'background-removed.png');
            return;
        }
        await removeBackground();
    });

    premiumDownloadBtn.addEventListener('click', async (e) => {
        if (lastResultUrl) {
            downloadUrl(lastResultUrl, 'background-removed.png');
            return;
        }
